...
class SilentPackageInstallDetector(BaseScannerPlugin):
    ...
    def description(self) -> str:
        return "Detects dynamic background package installations executed within agent tools or MCP servers"
    ...
    def scan(self, files: List[str], config: Any) -> List[Dict[str, Any]]:
        ...
        for file_path in files:
            ...
            # Scoped strictly to tools or MCP server files
            is_tool_file = 'tool' in file_path.lower() or 'mcp' in file_path.lower()
            
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()

                # If it's not a tool file by path, verify if it defines tool decorators or imports BaseTool
                if not is_tool_file:
                    tool_indicators = ['@tool', '@mcp_tool', '@mcp.tool', '@m.tool', 'BaseTool', 'mcp.tool', 'mcp_tool']
                    if not any(kw in content for kw in tool_indicators):
                        continue

                # Fast pre-filtering: check for installers
...
class PackageInstallVisitor(ast.NodeVisitor):
    ...
    def _resolve_string_val(self, node) -> str:
        val, _ = self._resolve_command(node)
        return val
            ...
            if is_valid_exec and len(node.args) > 0:
                ...
                if self.install_pattern.search(command_str):
                    ...
                    if is_installation:
                        ...
                        is_unpinned = False
                        # If pip install or npm install is done, check if version constraint exists
                        if 'pip' in command_str or 'uv' in command_str:
                            ...
                            for p in packages:
                                # Skip arguments that are not package names
                                if p in ['install', 'requirements.txt', '.']:
                                    continue
                                if 'requirements' in p or p.endswith('.txt'):
                                    continue
