...
class ContextInflationDetector(BaseScannerPlugin):
    ...
    def scan(self, files: List[str], config: Any) -> List[Dict[str, Any]]:
        ...
        for file_path in files:
            ...
            if '.min.' in filename:
                continue
            # Exclude other JSON files unless it is package.json (used by Next.js/presets)
            if ext == '.json' and filename != 'package.json':
                continue
            # Skip media, binary, and large structured files
            if ext in ['.png', '.jpg', '.jpeg', '.gif', '.pdf', '.zip', '.tar', '.gz', '.exe', '.dll', '.so', '.pyc'] or ext in excluded_extensions:
                continue
            # Explicit exclusion for tokenizer configs and vocabularies
            if 'tokenizer' in filename or 'vocab' in filename:
                continue
    ...
    def _scan_content(self, file_path: str, content: str) -> List[Dict[str, Any]]:
        findings = []

        # 1. Invisible Character Flooding Detection
        # Combined Unicode range class to eliminate expensive alternation backtracking
        zw_chars_class = r'[\u200b-\u200f\ufeff\u202a-\u202e\u2060-\u2069\u180e\u00ad\ufe00-\ufe0f\u200a\u202f\u205f\u3000\U000e0020-\U000e007f\U000e0100-\U000e01ef\U0001d173-\U0001d17a]'
                    ...
                    while i < words_len - 2 * n + 1:
                        ...
                        if words[i] == words[i + n] and words[i : i + n] == words[i + n : i + 2 * n]:
                            ...
                            if run_count > 30:
                                ...
                                if not is_cjk_phrase and all(len(w) < 4 for w in words[i : i + n]):
                                    i += 1
                                    continue
        ...
        if len(lines) >= 15:
            ...
            for idx, line_raw in enumerate(lines):
                ...
                # Strip comment prefixes to analyze actual repeated text inside comments
        ...
        if total_pad_tokens > 50:
            ...
            ] or filename in ['dockerfile', 'makefile', 'jenkinsfile', 'gemfile', 'pipfile', 'readme', 'license']
