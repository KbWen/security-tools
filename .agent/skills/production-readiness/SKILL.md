...
> security-tools note: §5.2 (Error Surface Rules) in `engineering_guardrails.md` already
> bans silent `catch {}` with no logging — this checklist extends it to "is the log sink
> production-observable, not just debug?"
...
The rollback plan (per `engineering_guardrails.md` §2.1 Small & Reversible Changes) must answer:
...
- **auth-security**: Security errors are especially critical to observe in production. `engineering_guardrails.md` §5.2 (Error Surface Rules) applies with elevated priority to auth error paths.
