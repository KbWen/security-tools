...
foreach ($file in @($platformDoc, $claudePlatformDoc, $examplesDoc, $projectAgentsFile, $projectClaudeFile, $rootDeploySh, $rootDeployPs1, $rootDeployCmd, $canonicalDeploySh, $canonicalDeployPs1, $canonicalValidateSh, $canonicalValidatePs1, $canonicalAuditSh, $textIntegrityCheckPs1, $textIntegrityBaseline)) { Assert-PathExists -Path $file -Message "missing required file: $file" }
...
foreach ($pattern in @('# AgentCortex Template - Downstream Ignore Defaults','.agentcortex/context/work/*.md','.agentcortex/context/private/','.agent/private/','.agentcortex-src/','*.acx-incoming','.openrouter/','.claude-chat/','.cursor/','.antigravity/scratch/','# End AgentCortex Template - Downstream Ignore Defaults')) {
    if ($deployBlock -notcontains $pattern) { Write-Error "deploy ignore block missing pattern: $pattern"; exit 1 }
}
foreach ($pattern in @('.agent/', '.agents/', 'AGENTS.md', 'CLAUDE.md')) {
